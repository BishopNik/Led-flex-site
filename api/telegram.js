...
module.exports=async function handler(request,response){
	...
	const {name,email,phone,country,message,language,source,website}=request.body||{};
	...
	const text=['✨ Nowe zapytanie — LEDFlex','Odbiorca: @NikanorovAn','',`👤 ${clean(name)}`,`✉️ ${clean(email)}`,phone?`📞 ${clean(phone)}`:null,`📍 ${clean(country)}`,`🌐 ${clean(language).toUpperCase()}`,'',`💬 ${clean(message)}`,'',source?`Źródło: ${clean(source)}`:null].filter(Boolean).join('\n');
